...
public class GuiSchematicManager extends GuiSchematicBrowserBase implements ISelectionListener<DirectoryEntry>
{
    ...
    private void createButtons()
    {
        ...
        this.addButton(new ButtonGeneric(this.width - buttonWidth - 10, y, buttonWidth, 20, label), new GuiMainMenu.ButtonListenerChangeMenu(type, null));
    }
    ...
    private static class ButtonListener implements IButtonActionListener
    {
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton)
        {
            ...
            else if (this.type == Type.RENAME_SCHEMATIC)
            {
                ...
                openGui(new GuiTextInputFeedback(256, "litematica.gui.title.rename_schematic", oldName, this.gui, new SchematicRenamer(entry.getDirectory(), entry.getName(), this.gui)));
            }
            else if (this.type == Type.DELETE_SCHEMATIC)
            {
                FileDeleter deleter = new FileDeleter(entry.getFullPath());
                openGui(new GuiConfirmAction(400, "litematica.gui.title.confirm_file_deletion", deleter, this.gui, "litematica.gui.message.confirm_file_deletion", entry.getName()));
            }
