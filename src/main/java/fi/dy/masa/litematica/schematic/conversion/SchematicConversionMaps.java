...
public class SchematicConversionMaps
{
    ...
    public static void computeMaps()
    {
        ...
        for (ConversionData data : CACHED_DATA)
        {
            try
            {
                ...
                {
                    CompoundNBT oldStateTag = getStateTagFromString(data.oldStateStrings[0]);
                ...
                CompoundNBT newStateTag = getStateTagFromString(data.newStateString);
    ...
    public static CompoundNBT get_1_13_2_StateTagFor_1_12_Tag(CompoundNBT oldStateTag)
    {
        CompoundNBT tag = OLD_STATE_TO_NEW_STATE.get(oldStateTag);
        return tag != null ? tag : oldStateTag;
    }

    public static CompoundNBT get_1_12_StateTagFor_1_13_2_Tag(CompoundNBT newStateTag)
    {
        CompoundNBT tag = NEW_STATE_TO_OLD_STATE.get(newStateTag);
    ...
    private static void addOverrides()
    {
        ...
        ID_META_TO_BLOCKSTATE.put(idOldLog | 0, Blocks.OAK_WOOD.getDefaultState().with(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(idOldLog | 1, Blocks.SPRUCE_WOOD.getDefaultState().with(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(idOldLog | 2, Blocks.BIRCH_WOOD.getDefaultState().with(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(idOldLog | 3, Blocks.JUNGLE_WOOD.getDefaultState().with(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(idNewLog | 0, Blocks.ACACIA_WOOD.getDefaultState().with(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(idNewLog | 1, Blocks.DARK_OAK_WOOD.getDefaultState().with(RotatedPillarBlock.AXIS, Direction.Axis.Y));
    ...
    private static void addIdMetaToBlockState(int idMeta, CompoundNBT newStateTag, String... oldStateStrings)
    {
        try
        {
            ...
            {
                CompoundNBT oldStateTag = getStateTagFromString(oldStateStrings[0]);
    ...
    private static void addOldStateToNewState(CompoundNBT newStateTagIn, String... oldStateStrings)
    {
        try
        {
                ...
                if (oldStateTag != null && newStateTagIn.getKeys().equals(oldStateTag.getKeys()))
                {
                    ...
                    if (newBlockName != null && newBlockName.equals(oldBlockName) == false)
                    {
                        for (String oldStateString : oldStateStrings)
                        {
                            ...
                            if (oldStateTag != null)
                            {
                                CompoundNBT newTag = oldStateTag.copy();
