...
public class LitematicaSchematic
{
        ...
        for (String regionName : tag.getKeys())
        {
            if (tag.get(regionName).getType() == Constants.NBT.TAG_COMPOUND)
            {
                ...
                if (regionPos != null && regionSize != null)
                {
                    ...
                    if (regionTag.contains("BlockStates", Constants.NBT.TAG_LONG_ARRAY) &&
                        regionTag.contains("BlockStatePalette", Constants.NBT.TAG_LIST))
                    {
    ...
    {
        return new Vector3i(tag.getInt("Width"), tag.getInt("Height"), tag.getInt("Length"));
    }
    ...
    {
        Map<BlockPos, CompoundNBT> blockEntities = new HashMap<>();
        ...
        for (int i = 0; i < size; ++i)
        {
            CompoundNBT beTag = tagList.getCompound(i);
    ...
    public boolean readFromSpongeSchematic(String name, CompoundNBT tag)
    {
        ...
        Vector3i size = readSizeFromTagSponge(tag);
    ...
    private boolean readFromFile(FileType schematicType)
    {
        try
        {
            CompoundNBT nbt = readNbtFromFile(this.schematicFile);
    ...
    {
        CompoundNBT nbt = readNbtFromFile(fileFromDirAndName(dir, fileName, FileType.LITEMATICA_SCHEMATIC));
