...
public class SchematicWorldRefresher implements IRangeChangeListener
{
    ...
    public void updateBetweenY(int minY, int maxY)
    {
        ...
        if (world != null)
        {
            ...
            for (Chunk chunk : schematicChunks.values())
            {
                ...
                {
                    renderer.markBlockRangeForRenderUpdate((chunk.x << 4) - 1, minY, (chunk.z << 4) - 1, (chunk.x << 4) + 16, maxY, (chunk.z << 4) + 16);
                }
            }
        }
    }
    ...
    public static void markSchematicChunksForRenderUpdate(ChunkPos chunkPos)
    {
        ...
        if (world != null)
        {
            ...
            {
                WorldRenderer renderer = LitematicaRenderer.getInstance().getWorldRenderer();
                renderer.markBlockRangeForRenderUpdate((chunkPos.x << 4) - 1,   0, (chunkPos.z << 4) - 1,
